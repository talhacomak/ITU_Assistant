...
public class frCarsambajava extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if(getArguments() != null){
            ...
            share.remove(getActivity(), "Carsamba");
            share.remove(getActivity(), "Carsamba_Saat");
            share.save(getActivity(), "days", days_ar);
            share.save(getActivity(), "Carsamba", mySet);
            share.save(getActivity(), "Carsamba_Saat", mySet2);

        }
        else{
            shared_pref sharenew = new shared_pref();
            days_ar = sharenew.getStringArray(getActivity(),"days");
            Set<String> dersAdiSet = sharenew.getStringSet(getActivity(), "Carsamba");
            Set<String> dersSaatiSet = sharenew.getStringSet(getActivity(), "Carsamba"+"_Saat");
        ...
        for(int i=0; i<strArray.length; i++){
            if(strArray[i] == null || !days_ar[i].equals("Carsamba")) continue;
