...
public class frPersembejava extends Fragment {
    ...
    public frPersembejava(){

    }

    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.persembe, container, false);
        ...
        if(getArguments() != null){
            ...
            share.remove(getActivity(), "Persembe");
            share.remove(getActivity(), "Persembe_Saat");
            share.save(getActivity(), "days", days_ar);
            share.save(getActivity(), "Persembe", mySet);
            share.save(getActivity(), "Persembe_Saat", mySet2);

        }
        else{
            shared_pref sharenew = new shared_pref();
            days_ar = sharenew.getStringArray(getActivity(),"days");
            Set<String> dersAdiSet = sharenew.getStringSet(getActivity(), "Persembe");
            Set<String> dersSaatiSet = sharenew.getStringSet(getActivity(), "Persembe"+"_Saat");
        ...
        for(int i=0; i<strArray.length; i++){
            if(strArray[i] == null || !days_ar[i].equals("Persembe")) continue;
